//! Interactive terminal UI for agent `ask_question` prompts.
mod helpers;
mod render;
mod session;
mod state;
...
use helpers::{insert_text, reserve_space, CANCEL_CONFIRM_WINDOW, MAX_PANEL_LINES};
use render::draw;
use session::QuestionSession;
use state::{handle_editing_key, submitted_answers, QuestionState};
...
mod tests {
    use super::helpers::{
        editor_view, insert_text, strip_ansi, truncate_width, MAX_PANEL_LINES,
    };
    use super::render::{editor_option_line, option_lines, panel_layout};
    use super::session::QuestionSession;
    use super::state::{handle_editing_key, submitted_answers, QuestionState};
    use crate::question::{
        QuestionOption, QuestionPrompt, QuestionRequest, MAX_CUSTOM_ANSWER_CHARS,
    };
