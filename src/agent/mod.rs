...
// ---------------------------------------------------------------------------
// Shared helpers for the agent tool-calling loop
// ---------------------------------------------------------------------------

/// Create a pre-ticked spinner interval (avoids the 3-line boilerplate).
fn new_spinner_interval() -> tokio::time::Interval {
    let mut interval = tokio::time::interval(SPINNER_INTERVAL);
    interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
    interval.tick();           // consume the immediate first tick
    interval
}

/// Emit a `ToolResult { ok: false, ... }` event and return the formatted error string.
fn emit_tool_error<F>(
    event_name: &str,
...
/// Emit a `ToolResult { ok: true, ... }` event.
fn emit_tool_result<F>(
    event_name: &str,
    output: &str,
    on_event: &mut F,
) -> Result<()>
...
mod tests {
    ...
    fn mode_reminder_does_not_inject_a_reasoning_title_protocol() {
        let paths = test_paths(std::path::Path::new("/tmp/gqy-test-agent-mode"));
