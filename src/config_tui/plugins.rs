...
use super::widgets::{
    display_width, draw_box, pad, parse_bool_field, parse_key_list, read_key, run_form, truncate,
    Field,
};
...
fn plugin_fields(config: &AppConfig, index: usize) -> Vec<Field> {
    match index {
        ...
        9 => vec![
            ...
            Field::boolean(
                t("Automatic skill learning", "自动技能学习"),
...
fn apply_plugin_fields(config: &mut AppConfig, index: usize, fields: &[Field]) -> Result<()> {
    match index {
        ...
        9 => {
            ...
            config.plugins.memory.forgetting_enabled = parse_bool_field(&fields[9].value)?;
