...
### 修复
- 面板切换模型后无限循环「检测到配置变更」+ 界面每 2 秒刷新：
  配置热重载（config watcher）检测到文件变化 → 重载后会把规范化配置写回
  文件（mtime 再次变化）→ 又触发重载……死循环。现在等应用完成后再把
  写入后的 mtime 设为基线，吸收自我写入；同时面板自己保存/切换模型
  触发的一次重载改为静默同步（不再弹提示），外部修改配置仍正常提示
